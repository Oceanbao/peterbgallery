	...
	>
		<nav class="flex flex-col items-center gap-8 font-montserrat text-3xl text-gray-50">
			...
			<div class="flex flex-col items-center">
				<button
					in:fly={{ y: -200, easing: circOut, duration: 400, delay: 400 }}
					on:click={() => (showWorks = !showWorks)}
					class="relative"
				>
					<span>works</span>
					{#if showWorks}
						...
						<svg
							transition:fade
							class="absolute left-[-40%] top-[5%] text-primary-light"
							width="30"
							height="30"
							viewBox="0 0 15 15"
							fill="none"
					...
					{:else}
						...
						<svg
							transition:fade
							class="absolute left-[-40%] top-[10%] text-primary-light"
							width="30"
							height="30"
							viewBox="0 0 15 15"
							fill="none"
				...
				<div
					class={`text-thin relative mt-4 flex max-h-0 flex-col items-center gap-2 overflow-hidden text-lg transition-all duration-700 ease-linear ${
						showWorks ? 'max-h-full' : ''
					}`}
				>
					<a href="/pen" on:click={closeSideMenu}>pen</a>
					<a href="/water" on:click={closeSideMenu}>water</a>
					<a href="/cink" on:click={closeSideMenu}>chinese ink</a>
					<a href="/acrylic" on:click={closeSideMenu}>acrylic</a>
					<a href="/printing" on:click={closeSideMenu}>printing ink</a>
					<a href="/oil" on:click={closeSideMenu}>oil</a>
				</div>
			</div>
		</nav>
		<div class="mt-8 grid place-content-center place-items-center">
			...
			<a href="https://www.instagram.com/peterbgallery/" target="_blank" rel="noreferrer">
				<svg
					class="mt-8 h-6 w-6 text-white lg:h-8 lg:w-8"
					viewBox="0 0 15 15"
					fill="none"
					xmlns="http://www.w3.org/2000/svg"
					><path
